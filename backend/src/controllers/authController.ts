...
export async function register(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (inviteCode) {
      ...
      console.log("✅ Organization found. ID:", organizationId);
    } else {
      console.log("🏢 Creating new organization:", organizationName);
      
      // ✅ FIX: Explicitly generate invite code
      ...
      const { data: org, error: orgError } = await supabase
        ...
        .insert({ 
          ...
          inviteCode: newInviteCode  // ✅ Explicitly set
        })
        .select()
        .single();
      ...
      if (orgError) {
        ...
        throw new ApiError(500, `Organization creation failed: ${orgError.message}`);
      }
      organizationId = org.id;
    ...
    if (userError) {
      ...
      throw new ApiError(500, `User creation failed: ${userError.message}`);
    }
